...
<div class="container-fluid">
  ...
  <div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Timetable Edit Form
    </h1>
  </div>
  <div class="container">
    <form action="{{route('timetable.update',$timetables->id)}}" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group row {{ $errors->has('day') ? 'has-error' : '' }}">
            <label for="inputAcademic" class="col-sm-2 col-form-label">Day</label>
            <div class="col-sm-5">
              <select class="form-control form-control-md" id="inputAcademic" name="day">
                <optgroup label="Choose Academic">
                  @foreach($grades as $row)
                <option value="{{$row->id}}" @if($grades->id == $row->id){{'selected'}} @endif>{{$row->name}}
                </option>
              @endforeach
          </optgroup>
              </select>
